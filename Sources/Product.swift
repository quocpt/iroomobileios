...
public struct Product: Mappable {
    ...
    public static func get(id: Int, completion: (_ success: Bool, product: Product?) -> Void) {
        let client = Client.sharedClient
        client.get("product", id: id, completion: completion)
    }

    public static func getAll(byCategory category: ProductCategory, limit: Int = 10, completion: (success: Bool, _ orders: [Product]?) -> Void) {
        let client = Client.sharedClient
